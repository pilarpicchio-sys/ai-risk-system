import numpy as np
import pandas as pd
import matplotlib.pyplot as plt
from src.optimization.cvar_position_sizing import compute_cvar_sizes

from sklearn.ensemble import RandomForestRegressor

from src.strategy.position_sizing1 import compute_position_sizes


# =========================
# LOAD
# =========================

df = pd.read_csv("data/sp500_multi_horizon.csv")
df = df.dropna().reset_index(drop=True)


# =========================
# FEATURES
# =========================

feature_cols = [
    "momentum_5",
    "momentum_20",
    "momentum_60",
    "vol_20",
    "vol_60",
    "trend_50",
    "trend_200",
    "zscore_20"
]

X = df[feature_cols]


# =========================
# TARGETS
# =========================

targets = {
    "5d": df["target_5d"],
    "21d": df["target_21d"],
    "63d": df["target_63d"]
}


# =========================
# SPLIT
# =========================

split = int(len(X) * 0.7)

X_train, X_test = X[:split], X[split:]

returns = df["return"].values
returns_test = returns[split:]


# =========================
# MULTI-HORIZON MODELS
# =========================

preds_dict = {}

for name, y in targets.items():

    y_train = y[:split]
    y_test = y[split:]

    model = RandomForestRegressor(
        n_estimators=200,
        max_depth=6,
        random_state=42
    )

    model.fit(X_train, y_train)

    preds_dict[name] = model.predict(X_test)


# =========================
# COMBINE (STATIC)
# =========================

combined = (
    0.2 * preds_dict["5d"] +
    0.5 * preds_dict["21d"] +
    0.3 * preds_dict["63d"]
)


# =========================
# 🔴 QUANTILE (CONFIDENCE)
# =========================

y_train = df["target_21d"][:split]

q10 = np.quantile(y_train, 0.1)
q90 = np.quantile(y_train, 0.9)

confidence = np.clip((combined - q10) / (q90 - q10), 0, 1)


# =========================
# FINAL SIGNAL
# =========================

final_signal = combined * confidence


# =========================
# POSITION SIZING
# =========================

# =========================
# BASE (V3)
# =========================

sizes_v3 = compute_position_sizes(final_signal, df)

# =========================
# CVAR
# =========================

sizes_cvar = compute_cvar_sizes(final_signal, returns_test)

# =========================
# 🔴 BLEND
# =========================

sizes = 0.7 * sizes_v3 + 0.3 * sizes_cvar


# =========================
# BACKTEST
# =========================

equity = 1.0
equity_curve = []

for i in range(len(sizes) - 1):
    equity *= (1 + sizes[i] * returns_test[i + 1])
    equity_curve.append(equity)

equity_curve = np.array(equity_curve)


# =========================
# BUY & HOLD
# =========================

bh_equity = 1.0
bh_curve = []

for r in returns_test:
    bh_equity *= (1 + r)
    bh_curve.append(bh_equity)

bh_curve = np.array(bh_curve)


# =========================
# PERFORMANCE
# =========================

max_drawdown = np.min(equity_curve / np.maximum.accumulate(equity_curve) - 1)

print("\n=== V3 FINAL ===")
print("Strategy equity:", equity)
print("Buy&Hold equity:", bh_equity)
print("Max drawdown:", max_drawdown)


# =========================
# PLOT
# =========================

plt.figure(figsize=(10, 5))
plt.plot(equity_curve, label="V3 Final")
plt.plot(bh_curve, label="Buy & Hold")
plt.legend()
plt.title("Full V3 Engine")
plt.show()