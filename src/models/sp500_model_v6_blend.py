...
def make_decisions(preds_z, df):
    ...
    for i in range(len(preds_z)):
        ...
        size_v5 = (0.6 + z * 0.35) * risk_scale

        # 🔥 BLEND
        size = 0.6 * size_v3 + 0.4 * size_v5
...
plt.plot(equity_curve, label="Strategy V6")
plt.plot(bh_curve, label="Buy & Hold")
plt.legend()
plt.title("Blended Risk Strategy")
plt.show()